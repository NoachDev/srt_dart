...
class SrtAddress {
  ...
  static ffi.Pointer<sockaddr> createIpv4Address(
    String hostAddress,
    int port,
    ...
    try {
      ...
      try {
        ...
        final sinAddrPtr = addr
            .cast<ffi.Uint8>()
            .elementAt(ffi.sizeOf<sa_family_t>() + ffi.sizeOf<in_port_t>())
            .cast<in_addr>();
            
        final result = bindings.inet_pton(
          AddressFamily.inet,
  ...
  static ffi.Pointer<sockaddr> createIpv6Address(
    ...
    int flowInfo = 0,
    int scopeId = 0,
    required srt_dart_bindings bindings,
  }) {
    if (port < 0 || port > 65535) {
      throw ArgumentError('Port must be between 0 and 65535, got $port');
    }
    ...
    try {
      ...
      try {
        ...
        final sin6AddrPtr = addr
            .cast<ffi.Uint8>()
            .elementAt(ffi.sizeOf<sa_family_t>() + ffi.sizeOf<in_port_t>() + ffi.sizeOf<ffi.Uint32>())
            .cast<in6_addr>();
  ...
  static ffi.Pointer<sockaddr> fromInternetAddress(
    ...
    switch (address.type) {
      case InternetAddressType.IPv4:
        return createIpv4Address(address.address, port, bindings);
